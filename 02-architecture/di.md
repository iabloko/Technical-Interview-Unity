...
public class Game
{
    private readonly SaveSystem _save = new SaveSystem();          // прибили гвоздями
...
| **Field injection** | Самый «магический», ломает тестируемость без контейнера. На проде избегаем. |
...
- **Конструкторы должны быть «тупыми».** Конструктор только присваивает ссылки — никаких загрузок ассетов, парсинга JSON, сетевых вызовов. Тяжёлое выносим в `IInitializable.Initialize()` (а лучше — в `async`/UniTask) и растягиваем по кадрам, чтобы не морозить главный поток.
...
- **Крайняя мера — сменить контейнер.** Если рефлексия Zenject стала бутылочным горлышком, **VContainer** (source-gen, без рантайм-рефлексии) прогревается быстрее. Это архитектурное решение, не «на горячую».

### 3. Перекрёстные (циклические) зависимости: A требует B, B требует A

**Сначала — это запах дизайна.** Цикл почти всегда означает, что ответственности размазаны между двумя классами. Правильное лечение — убрать сам цикл, а не «разрулить» его технически:
...
> На собесе ценится именно такой порядок ответа: **сначала** «это сигнал к рефакторингу — выделить посредника/событие», и **только потом** «технически Zenject разрывает цикл через method-injection или `Lazy<T>`». Кандидат, который сразу лезет за `Lazy<T>`, лечит симптом.
