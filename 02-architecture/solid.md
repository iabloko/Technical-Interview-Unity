...
> Модули верхнего уровня **не зависят** от модулей нижнего уровня — оба зависят от **абстракций**. Абстракции не зависят от деталей — детали зависят от абстракций.

Инвертируется направление зависимости: интерфейс (`ISaveSystem`) объявляет модуль верхнего уровня под свои потребности, а реализация нижнего уровня (`FileSaveSystem`) зависит от этого интерфейса. DIP — принцип проектирования; Dependency Injection — способ его реализовать (реализация передаётся снаружи); DI-контейнер — инструмент для DI.
...
- Расшифровка SOLID и пример нарушения каждого принципа.
- SRP: что считается «причиной для изменения».
- OCP: как добавить поведение без правки существующего кода.
- LSP: почему `NotSupportedException` в переопределённом методе наследника — нарушение.
- ISP: признаки «толстого» интерфейса.
- Чем DIP отличается от Dependency Injection и DI-контейнера.
