...
- **Параметрический** — обобщения (generics): единый алгоритм для любого типа `T` (`List<T>`, `Comparer<T>`). CLR специализирует машинный код под каждый value-тип и переиспользует одну реализацию для всех ссылочных типов.
...
> Принцип: **«Предпочитай композицию наследованию»** (из книги «Банды четырёх» — Gang of Four, «Design Patterns», 1994).

## Что спрашивают на собеседовании

- Четыре принципа ООП и чем они выражаются в C#.
- Виды полиморфизма: перегрузка, переопределение, обобщения.
- Разница `override` и `new`: вызов по типу объекта или по типу ссылки.
- Модификаторы доступа, включая `protected internal` и `private protected`.
- Почему в Unity предпочитают композицию наследованию.
