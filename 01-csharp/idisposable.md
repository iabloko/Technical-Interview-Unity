...
`using` работает по **паттерну**, а не только по интерфейсу: компилятор вызовет `Dispose()`, если он есть, даже без реализации `IDisposable`. Это важно для `ref struct` (например, кастомных enumerator'ов), которые **не могут** реализовывать интерфейсы (боксинг запрещён), но участвуют в `using` и `foreach` через метод `Dispose()` по соглашению. Освобождение `struct` через `using` не боксит — ещё и поэтому это дешевле.
...
- Может ли `ref struct` реализовать `IDisposable`? (Нет — `using` работает по паттерну `Dispose()` без интерфейса.)
