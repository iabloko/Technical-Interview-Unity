...
- **Рефлексия медленная.** Поиск членов, `Invoke`, `CreateInstance` — на порядки дороже прямого вызова. Не использовать в hot path. Если нужна — кешировать `MemberInfo` и один раз создать делегат (`Delegate.CreateDelegate`), вызывая дальше его, а не `Invoke`. Компиляция expression tree (`Expression.Compile()`) ускоряет вызов только при JIT (Mono); под IL2CPP expression tree исполняется интерпретатором, выигрыша нет.
...
- Как ускорить рефлексию (кеш `MemberInfo`, `Delegate.CreateDelegate`; почему `Expression.Compile()` не помогает под IL2CPP).
