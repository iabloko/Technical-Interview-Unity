...
- `null` в поле обычного `[Serializable]`-класса: сериализация по значению не хранит `null` и подставляет экземпляр с полями по умолчанию (`[SerializeReference]` хранит `null`).
- Вложенность ограничена 10 уровнями; общие ссылки на один экземпляр `[Serializable]`-класса после десериализации становятся независимыми копиями, рекурсивные структуры упираются в лимит глубины (см. `[SerializeReference]`).
...
- Unity сериализует поля, а не свойства; данные привязаны к **именам полей** (переименование поля без `[FormerlySerializedAs]` теряет значение) и к **GUID скрипта** в `.meta` (переименование класса вместе с файлом сохраняет ссылки на компонент; потеря `.meta` их ломает).
