...
`defaultCapacity` задаёт только начальную ёмкость внутреннего стека и объекты не создаёт. `maxSize` — предел хранимых в пуле объектов: лишние при `Release` уничтожаются через `actionOnDestroy`.
...
- **Двойной `Release`** одного объекта ломает пул (объект выдадут дважды). Защищаться флагом/проверкой; у `ObjectPool<T>` параметр `collectionCheck` (по умолчанию `true`) бросает исключение при повторном возврате.
...
- Прогрев (prewarm) на загрузке, чтобы не платить за создание во время геймплея. У `ObjectPool<T>` встроенного прогрева нет: вызвать `Get` N раз и вернуть объекты через `Release`.
