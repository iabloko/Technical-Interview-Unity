...
- Как найти, что нагружает кадр (иерархия CPU, самый дорогой сэмпл).
- Почему замеры в Editor неточны и как профилировать билд (Development Build, Autoconnect Profiler).
- Как найти источник GC-аллокаций (колонка `GC Alloc`).
- Чем Deep Profile отличается от ручных `ProfilerMarker`.
- Как по разбивке времени кадра понять, упирается кадр в CPU или в GPU.
