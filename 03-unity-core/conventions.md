...
- **`[RequireComponent(typeof(T))]`** делает зависимость явной вместо `GetComponent` в `Start`.
...
- `CompareTag("Enemy")` вместо `tag == "Enemy"` (последнее аллоцирует строку на некоторых платформах).
...
## Асинхронность и текст (дефолты проектов)
